...
<Tabs>
  ...
  <Tab title="Server/DC (PAT headers)">
    ```json
    {
      "mcpServers": {
        "mcp-atlassian-service": {
          "url": "http://localhost:9000/mcp",
          "headers": {
            "X-Atlassian-Jira-Url": "<USER_JIRA_BASE_URL>",
            "X-Atlassian-Jira-Personal-Token": "<USER_JIRA_PAT>",
            "X-Atlassian-Confluence-Url": "<USER_CONFLUENCE_BASE_URL>",
    ...
    headers when both services should be available. The Atlassian URL headers are
    request-scoped too: the server uses the URL supplied in the same request as
    the PAT, not a shared Jira/Confluence default. The MCP server does not store
...
without `JIRA_URL`, `CONFLUENCE_URL`, `JIRA_PERSONAL_TOKEN`, or
`CONFLUENCE_PERSONAL_TOKEN`. Each request must provide a complete service header
pair:
